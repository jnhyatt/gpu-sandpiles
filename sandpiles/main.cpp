...
LRESULT CALLBACK windowsMessageCallback(HWND hWindow, UINT message, WPARAM wParam, LPARAM lParam)
{
  switch (message)
  {
  case WM_CLOSE:
...
int main()
{
  ...
  HWND hTmpWindow = CreateWindowEx(WS_EX_OVERLAPPEDWINDOW, "sandbox", "Sandbox", WS_OVERLAPPEDWINDOW, 80, 80, 1440, 900, NULL, NULL, hInstance, NULL);
  ...
  HWND hWindow = CreateWindowEx(
    ...
    windowSize.left, windowSize.top, windowSize.right, windowSize.bottom, NULL,
    NULL, hInstance, NULL);
  if (!hWindow)
  {
    log.fatal() << WindowsError::last();
  ...
  log.verbose() << "Selecting pixel format... ";
  if (!wglChoosePixelFormatARB(hDeviceContext, pixelFormatAttributes, nullptr, 1, &format, &formatCount))
  {
  ...
  DescribePixelFormat(hDeviceContext, format, sizeof(PIXELFORMATDESCRIPTOR), &formatDescriptor);
  ...
  HGLRC hRenderingContext = wglCreateContextAttribsARB(hDeviceContext, NULL, contextAttribs);
  if (!hRenderingContext)
  {
    log.fatal() << "Failed to create rendering context with the requested attributes. ";
    return NULL;
  }
  ...
  for (size_t i = 0; i < 2; i++) {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_R32UI, 16, 16, 0, GL_RED, GL_UNSIGNED_INT, texData);
  ...
  float screenQuad[8] { -1.0f, -1.0f, 1.0f, -1.0f, 1.0f, 1.0f, -1.0f, 1.0f };
  ...
  try
  {
    ...
    GLuint vShader = compile(loadFile("sandpile.vs").c_str(), GL_VERTEX_SHADER);
    log.verbose() << "Compiling sandpile.fs... ";
    GLuint fShader = compile(loadFile("sandpile.fs").c_str(), GL_FRAGMENT_SHADER);
  ...
  glUniform2f(glGetUniformLocation(sandpileProgram, "dim"), 16, 16); // texture dimensions

  GLuint displayProgram = glCreateProgram();
  try
  {
    log.verbose() << "Compiling display.vs... ";
    GLuint vShader = compile(loadFile("display.vs").c_str(), GL_VERTEX_SHADER);
    log.verbose() << "Compiling display.fs... ";
    GLuint fShader = compile(loadFile("display.fs").c_str(), GL_FRAGMENT_SHADER);
  ...
  if (!ShowWindow(hWindow, SW_SHOWDEFAULT))
  {
    log.error() << WindowsError::last();
  }
